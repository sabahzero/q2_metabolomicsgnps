...
This is a [Qiime2](https://qiime2.org/) plugin for analyzing metabolomics data by, utilizing [GNPS](https://gnps.ucsd.edu/ProteoSAFe/static/gnps-splash.jsp).
...
![](https://3.bp.blogspot.com/-5lKOIos0b30/WO-Fs3d108I/AAAAAAAALCY/YT8oWMqoUXct-ut0cCjLS6XJ1--FVI_QwCLcB/s1600/QE_MS3..JPG)
...
## Installation of q2 metabolomics plugin

#### First, install Qiime2 and activate the environment by following the steps described [here](https://docs.qiime2.org/2018.6/install/native/).

If you already have Qiime2 installed, source by
```
source activate qiime2-2018.6
```

To test if the Qiime2 installation was successful by typing the following command (options will appear)
...
#### Now, install the q2_metabolomics plugin using one of two options
...
Test if the plugin was installed correctly by repeating the following command (the metabolomics plugin should now be listed in the options)

```
qiime
```

If you have encountered any problems up to this stage, please submit an issue request [here](https://github.com/mwang87/q2_metabolomics/issues). This approach can be used for problems encountered throughout. 

### Plugin Commands Listing

#### List all commands and overview of functions
*Note: We have not yet uploaded the example data. Thus, testing these functions will not work at this point in the tutorial. 
...
```
Let's now try these functions through these sample data.
## Example Data

In this tutorial, we will utilize two sets of data to demonstrate the different analyses we can do with this plugin. We will download metabolomics data for use with the metabolomics plugin for qiime2. 

1. Cross-Sectional Data - Discrete cohorts of plant and animal sample types
2. Longitudinal Data - Time series data fermentation of milk to yogurt

These data and the download process are described below.

### Input Data Description and Download

Here we create a directory to hold the data, download the actual mass spectrometry data/metadata, and reorganize.

Note: The files contained within the folders “other” have been created for this example dataset. If you want to recreate the example analyses listed below with your own dataset, you will have to create your own [manifest.csv](link to manifest file description) and [metadata.txt](https://docs.qiime2.org/2018.6/tutorials/metadata/) files.
...
In order to utilize MS1 Peak Areas in Qiime2, we will need to use MZMine2. A detailed tutorial for feature finding with MZmine2 can be found [here](https://ccms-ucsd.github.io/GNPSDocumentation/featurebasedmolecularnetworking/).

After finding all features according to the tutorial above, perform the following steps to export the features and their respective quantifications to be compatible with this Qiime2 plugin with the command
